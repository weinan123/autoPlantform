    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (projectList.objects.filter(projectName=form.cleaned_data['projectName']).count() == 0):
                ...
                inter.save()
                scheduledata = projectschedule.objects.create(
...
def uploadCase(request):
    if request.method == 'GET':
        ...
        if (len(caseList) > 0):
            for i in range(0, len(caseList)):
                ...
                if body!="{}" and body != None:
                    bodys = json.loads(body)["datas"][0]["paramValue"]
                else:
                    bodys = body
                ...
                caselist = [casename, method, realhost, url, headers, bodys, t_id,
                            depend_caseId,
        ...
        modelname = modelname.decode("utf-8").replace('/', '_')
